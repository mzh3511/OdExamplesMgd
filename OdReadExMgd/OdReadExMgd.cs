...
namespace OdReadExMgd
{
  class DbDumper
  {
    ...
    static void dumpDimData(Dimension pDim, int indent)
    {
      ...
      if (pDim.Measurement >= 0.0)
      {
        writeLine(indent, "Formatted Measurement", pDim.FormatMeasurement(pDim.Measurement, pDim.DimensionText));
      }
    ...
    void dump(PolyFaceMesh pPoly, int indent)
    {
      ...
      {
        if (o is PolyFaceMeshVertex)
        {
          PolyFaceMeshVertex pVertex = (PolyFaceMeshVertex)o;
        ...
        else if (o is FaceRecord)
        {
          FaceRecord pFace = (FaceRecord)o;
          string face = "{";
          for (short i = 0; i < 4; i++)
          {
            if (i > 0)
...
*/
      ...
      using (DimStyleTable pTable = (DimStyleTable)pDb.DimStyleTableId.Open(OpenMode.ForRead))
      {
        ...
        foreach (ObjectId id in pTable)
        {
          ...
          using (DimStyleTableRecord pRecord = (DimStyleTableRecord)id.Open(OpenMode.ForRead))
          {
            ...
            writeLine(indent, "Extension Line Fixed Len", pRecord.Dimfxl);
            writeLine(indent, "Extension Line Fixed Len Enable", pRecord.Dimfxlon);
    ...
    public void dumpHeader(Database pDb, int indent)
    {
      ...
      writeLine(indent, "File DWG Version:", pDb.OriginalFileVersion);
